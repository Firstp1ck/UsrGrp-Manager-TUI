...
pub fn render_group_details(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let (name, gid, members) = match group { Some(g) => (g.name.clone(), g.gid, g.members.len()), None => (String::new(), 0, 0) };
    let text = format!("Group: {name}\nGID: {gid}\nMembers: {members}");
    let p = Paragraph::new(text).style(Style::default().fg(app.theme.text)).block(
        Block::default().title("Group Details").borders(Borders::ALL).border_style(Style::default().fg(app.theme.border)),
    );
...
pub fn render_group_members(f: &mut Frame, area: Rect, app: &mut AppState) {
    ...
    let rows = slice.iter().map(|m| Row::new(vec![Cell::from(m.clone())]).style(Style::default()));
    let widths = [Constraint::Percentage(100)];
    let header = Row::new(vec!["Members"]).style(Style::default().fg(app.theme.title).add_modifier(Modifier::BOLD));

    let table = Table::new(rows, widths)
        .header(header)
        .block(Block::default().title("Group Members").borders(Borders::ALL).border_style(Style::default().fg(app.theme.border)))
        .column_spacing(1);
    f.render_widget(table, area);
}
...
pub fn render_group_modal(f: &mut Frame, area: Rect, app: &mut AppState, state: &ModalState) {
    match state.clone() {
        ModalState::GroupsActions { selected, .. } => {
            ...
            let p = Paragraph::new(text).block(Block::default().title("Group actions").borders(Borders::ALL).border_style(Style::default().fg(app.theme.border)));
        ...
        ModalState::GroupAddInput { name } => {
            ...
            let p = Paragraph::new(msg).block(Block::default().title("Create group").borders(Borders::ALL).border_style(Style::default().fg(app.theme.border)));
            f.render_widget(Clear, rect);
            f.render_widget(p, rect);
        }
